...
def joinFile(conf):
    ...
    for root,dirs,files in os.walk(conf):
        for fn in files:
            fn=root+'/'+fn
            if fn.find('.svn')==-1 and fn.find('.css')!=-1 and fn.find('-srclist.css')==-1 and fn.find('-srclist-temp.css')==-1:
              
                fl=_joinFile(fn,[])
                tt=fn.split('.css')

                _tempFile=open(tt[0]+'-srclist-temp.css',mode='w',encoding='utf-8')
    ...
    for root,dirs,files in os.walk(conf):
        for fn in files:
            fn=root+'/'+fn
            if fn.find('-srclist-temp.css')!=-1 and fn.find('.svn')==-1:
                os.rename(fn,re.sub(r"-temp.css$",".css",fn))
